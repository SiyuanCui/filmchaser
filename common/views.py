...
def sh(request):
    ...
    sh = utils.input(request, "sh", "")
    id = int(utils.input(request, "id", 0))

    if sh == "0" or sh == "":
        sql = "UPDATE %s SET issh='1' WHERE id='%s' " % (tablename, id,)
    else:
        sql = "UPDATE %s SET issh='0' WHERE id='%s' " % (tablename, id,)

    commonModels.execute(sql)

    return utils.showSuccess(request, "Audit succeeded" if sh == '0' or not sh else 'Approval canceled')
...
def UploadFile(request):
    ...
    return_info = {
        ...
        'state': state,  # Upload status, SUCCESS will be returned when successful, and any other value will be returned to the picture upload box as is
...
def mod(request):
    if request.method == 'POST':
        ...
        if not all([oldPwd, newPwd2, newPwd]):
            return utils.showError(request, "Please fill in the original password or new password or confirm the password")
