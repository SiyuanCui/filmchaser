...
def getWhere(request, qs):
    if request.GET.get("title"):
        qs = qs.filter(title__icontains=request.GET.get("title"))
    if request.GET.get("column"):
        qs = qs.filter(column=request.GET.get("column"))
    if request.GET.get("director"):
        qs = qs.filter(director__icontains=request.GET.get("director"))
    if request.GET.get("actor"):
        qs = qs.filter(actor__icontains=request.GET.get("actor"))
